...
const ContactSupport = (props) => {
  ...
  const countries = ["Organization", "Organization 1", "Organization 2", "Organization 3"]
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor: '#F7F8F8', height: 54,
          alignItems: 'center',
          borderRadius: 5,
          marginTop: 10
        }} >
        ...
        />
        <Text style={[styles.subtitle, { marginLeft: 10 }]} >{item.item.name}</Text>
      </TouchableOpacity>
    )
  }
            ...
            >
              <View style={{ flexDirection: 'row' }} >
                <Image
                  ...
                  style={{ height: 20, width: 20 }}
                />
                <Text style={[styles.subtitle, { marginLeft: 10 }]} >{organization}</Text>
              </View>
              <FontAwesome
                name={isOpened ? 'chevron-up' : 'chevron-down'}
                color={'#3B2645'} size={14}
                style={{
                  position: 'relative',
                  right: -5
                }}
...
const mapDispatchToProps = (dispatch) => ({
  contactSupportRequest: (data, navigation) => dispatch(contactSupportRequest(data, navigation)),
});
